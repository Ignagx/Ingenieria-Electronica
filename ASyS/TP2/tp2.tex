...
\begin{enumerate}[label=\alph*),left=0pt]
  ...
  \begin{figure}[h]
    ...
    \begin{minipage}[h]{0.4\textwidth}
      ...
      \textit{Representación de $y_1(t)$}
    \end{minipage}
    \hspace{5mm}
    \begin{minipage}[h]{0.4\textwidth}
      \centering
      \includegraphics[width=1\textwidth]{./images/ej1.2.png}
      \textit{Representación de $y_2(t)$}
    \end{minipage}
  \end{figure}
  ...
  \item Determinar las ecuaciones de cinemáticas para la posición $y_1(t)$ y $y_2(t)$ en función del tiempo para cada
    ...
    Partimos primero de la ecuación que determina la posición de un cuerpo con aceleración uniforme:
    $$Y(t) = Y_0 + V_0 t + \frac{1}{2} g t^2$$
    Para ambos casos asumimos la velocidad inicial $V_0 = 0$, despreciamos la resistencia del aire y tomamos como valor
    de aceleración de la gravedad $g = 9.8 \frac{m}{s^2}$:
    $$y_1(t) = Y_0 - \frac{1}{2}9,8\frac{m}{s^2}t^2$$
    $$y_2(t) = \frac{Y_0}{2} - \frac{1}{2}9,8\frac{m}{s^2}t^2$$


    Como se puede observar, las masas no aparecen en las ecuaciones de $y_1(t)$ y $y_2(t)$, por lo que se deduce que
    las masas no intervienen en la descripción de las funciones.
  ...
  \item Calcular el tiempo de vuelo (total) $T$ del primer cuerpo, en función de $y_0$. Similarmente, calcular el
    ...
    $$T = \sqrt{\frac{2h}{g}}$$
    $$\tau = \sqrt{\frac{h}{g}}$$
    La relación aritmética entonces:
    ...
    factor de $\sqrt{2}$ queda $y_1(t) = Y_0 - \frac{1}{2}g(\sqrt{2}t)^2$. T y $\tau$ coinciden, es decir los tiempos 
    finales son iguales, pero no coinciden las funciones de posición. Es importante notar que, aunque existe esta 
    relación entre los tiempos de vuelo, no es físicamente correcto interpretar la posición $y_2(t)$ del segundo cuerpo
    como una versión escalada temporalmente de $y_1(t)$. Los dos cuerpos tienen trayectorias y tiempos de vuelo 
  ...
  \item Calcular el tiempo $t_0$ que le toma al primer cuerpo estar en la posición $\frac{1}{2} y_0$. ¿Qué relación 
    ...
    No se puede obtener $y_2(t)$ como una traslación temporal de $y_1(t)$, porque al desplazar temporalmente $y_1(t)$,
  ...
  \item \textbf{Reflexionar:} ¿Tiene sentido físico evaluar la señal $y_1$ en tiempos $t > T$? Similarmente, ¿tiene
    ...
    su vez tampoco tiene sentido evaluar las señales en tiempos negativos, menores a cero. Podríamos definir las
    señales como:
  ...
  \begin{itemize}[]
    ...
    Dado que las señales $y_1(t)$ e $y_2(t)$ tienen energía finita, se puede afirmar que tienen una potencia media
    igual a cero.
    \item Causalidad

    Ambas señales son causales por el hecho de estar multiplicadas por los escalones unitarios $(\mu(t)\mu(T - t)$ para
    $y_1(t)$ y $\mu(t)$ $\mu(\tau - t) $ para $ y_2(t)$) que anulan el pasado y futuro de las mismas.
...
\begin{enumerate}[label=\alph*),left=0pt]
  ...
  \item Determinar la ecuación de cinemática para la posición $x(t)$ en función del tiempo. \textbf{Reflexionar:} ¿la
    ...
    Partiendo de la ecuación diferencial de un oscilador armónico simple $m \frac{dx^2}{d^2t} + kx = 0$, determinamos
    la ecuación de cinemática para la posición en función del tiempo $x(t) = A \sin(\omega t + \phi)$. Si, la función
  ...
  \begin{itemize}
    \item Periódicidad
      ...
      Es periódica ya que existe un valor T tal que $x(t) = x (t + nT)$, donde n = 1,2,3..
    \item Energía

    Tiene energía infinita. Para señales periódicas, la energía total en el sentido clásico puede ser infinita debido a
    que la señal se repite indefinidamente, pero si hablamos de la energía en un periodo T, es finita y es:
    ...
    Tiene energía infinita si consideramos el intervalo $(-\infty,\infty)$, pero la energía es finita en un solo
    periodo, por lo que la potencia media es finita:
    ...
    La señal es acotada ya que para todo t, $x(t) \leq A$ siendo A la amplitud. Esto se da ya que las funciones

    Trigonométricas seno o coseno toman valores entre -1 y 1
    \item Causal
...
\begin{enumerate}[label=\alph*),left=0pt]
  ...
  \item Calcular el periodo fundamental $T_0$ de las señales $x_1, x_2$. ¿Las señales tienen energía finita? Calcular
    ...
    Energía relativa a un periodo $T_0$:
    $$
    \begin{aligned}
...
\includegraphics[width=1\textwidth]{images/ej2.2a.png}
  \end{minipage}
...
$$
	...
	\textbf{Reflexión:} ¿si los periodos $T_n$ de cada señal del conjunto básico son distintos, por qué la suma
      (finita/infinita) es periódica? \\
		  Nuevamente las señales son periódicas con periodo $T_n=\frac{T_0}{n}$, de tal forma todas comparten el periodo $T_0$, y la suma finita/infinita de señales periódicas con un periodo común es periódica con dicho periodo.\\
		  ¿Por qué se restringe el dominio del tiempo en un intervalo del tipo 
      $t_0 \leq t \leq t_0 + T_0$, si las señales básicas están bien definidas en todo el eje temporal?\\
 ...
 \centering
  \begin{minipage}[h]{0.6\textwidth}
      \includegraphics[width=1\textwidth]{images/ej.2.5}
  \end{minipage}
...
\\
 ...
 \centering
  ...
  \item Comparar los espectros obtenidos para cada una de las señales $x_1, x_2$, evaluar y proponer las 
    ...
    la señal rectificada de onda completa todos los armónicos tienen 
...
\begin{enumerate}[label=\alph*),left=0pt]
  ...
  \item Realizar el gráfico de las señales $x_1 = G_1(t)$ y $x_2 = G_{0.1}(t)$, luego, calcular la energía $E_1$ de 
    ...
    \begin{figure}[h!]
      ...
      \begin{minipage}{0.45\textwidth}
        ...
        \caption{Gráfico de la señal $x_1$}
        \label{fig:imagen1}
      \end{minipage}
      \hfill
      \begin{minipage}{0.45\textwidth}
        \includegraphics[width=\textwidth]{images/ej3.2}
        \caption{Gráfico de la señal $x_2$}
        \label{fig:imagen2}
      \end{minipage}
    \end{figure}

    Para calcular la energía de cada respectiva señal aplicamos la definición:
    $$E_x = \int_{-\infty}^{\infty}x_t^2dt$$
    En este caso, por ser una señal de amplitud 1, simplemente es el área encerrada bajo cada una de las curvas
  ...
  \item Obtener la transformada de Fourier $F_1(\omega) = \mathcal{F}\{x_1\}$ y similarmente $F_2(\omega) =
    ...
    La transformada de Fourier para una un pulso genérico $G_T$ es:
    ...
    \begin {itemize}[left=0pt]
      ...
      \item Calcular la energía $E_1, E_2$ haciendo uso de la relación de Parseval.
        ...
        Siendo la energía de cada señal a través de la relación de Parseval:
        ...
        En este caso, el método sobre la variable $t$ es mucho más simple, directo y visual. En contra parte el calculo
        mediante la relación de Parseval es mucho más tedioso, requiriendo la resolución de una integral compleja
        y la ayuda de herramientas como la función $Si()$. Sin embargo, esto se debe a las características de nuestra
  ...
  \item Comparar los espectros obtenidos para cada una de las señales $x_1, x_2$, evaluar y proponer las 
    ...
    Se puede observar claramente en los gráficos que los espectros obtenidos obedecen a la relación:
...
\chapter{Ejercicio 4}
  ...
  \begin{figure}[h]
    ...
    \begin{minipage}[h]{0.5\textwidth}
      \centering
      \includegraphics[width=1\textwidth]{./images/ej4.1.jpeg}
      \textit{Circuito RC}
    \end{minipage}
  \end{figure}
  ...
  \begin{enumerate}[label=\alph*)]
    ...
    \item Obtener una representación del circuito con Ecuaciones Diferenciales de primer orden y coeficientes
      ...
      El circuito RC esta descrito por la ecuación característica:
      $$v(t) = R\cdot i(t) + \frac{1}{C} \cdot \int_{0}^{t} i(\tau) d\tau$$
      Para tener una nomenclatura más genérica, sustituimos la entrada $v(t)$ con $x(t)$ y la salida $i(t)$ con $y(t)$:
      ...
      Reacomodando la ecuación característica generalizada, nos queda:
      $$
      \begin{aligned}
      ...
      Lo cual es la ecuación diferencial de primer orden a coeficientes constantes que describe el comportamiento del
      circuito RC.

      Para el diagrama de simulación, utilizamos la ecuación característica generalizada del sistema, solo que se
      despeja y:
      \begin{figure}[h]
        \centering
        \begin{tikzpicture}[auto, node distance=2cm, >=Stealth, thick]
          ...
          \draw [-] (input) -- node {$x_t$} (elbow1);
          \draw [-] (elbow1) -- (gain1);
          \draw [->] (gain1) -- (sum);
          \draw [->] (sum) -- node {$y_t$} (output);
          \draw [->] (integrator) -- (sum);
    ...
    \item Teniendo en cuenta el siguiente resultado matemático, obtener una descripción explicita
      ...
      \begin{itemize}
        ...
        Por lo tanto, la solución general explicita para el $S_1\{x_t\}$ es:
    ...
    \item Usando la descripción explicita de la salida del Sistema $y_t = S_1{x_t}$ (CI arbitrarias) del inciso
      ...
      $t > 0$. Seria nula en caso de que las CI. sean nulas, o que se cumpla que $y_0 = x_0/R$.\\
    ...
    \item Usando la descripción explicita de la salida del Sistema $y_t = S_1\{x_t\}$ (CI arbitrarias), demostrar si el
      ...
      \textbf{Invarianza en el tiempo:} La verificación de la invarianza en el tiempo involucra realizar un desplazamiento
      ...
      a los limites de integración, ya que el resto de los términos son prácticamente iguales, y por el termino no
      lineal que involucra las condiciones iniciales.

    \fakesubsection{}
    \item Demostrar que el sistema anterior en reposo gana propiedades, esto es, definir $y_t = S_2\{x_t\}$ con CI. 
...
%      Invarianza en el tiempo: Trayendo nuevamente el análisis del punto anterior:
%      \begin{align*}
%        y_1(t) &= x_{(t - t_0)} \cdot \frac{1}{R} + \left(y_0 - \frac{x_0}{R}\right) \cdot
%          e^{\frac{-t}{RC}} - \frac{1}{R^2C} \int_{0}^{t} x_{(\tau - t_0)} \cdot e^{\frac{-(t-\tau)}{RC}} d\tau\\[12pt]
%        y_2(t) &= x_{(t - t_0)} \cdot \frac{1}{R} + \left(y_0 - \frac{x_0}{R}\right) \cdot e^{\frac{-(t-t_0)}{RC}} -
...
%        y_1(t) \neq y_2(t)
%      \end{gather*}
%      Lo que aun no permite que el sistema sea invariante en el tiempo, por los limites de integración que son
%      diferentes.

    \fakesubsection{}
    \item Refinar aun más el sistemas, exijamos CI nulas y respuesta causal (se elimine el pasado), esto es, definir
      ...
      Para el $S_3$, nuevamente trayendo desarrollos de los puntos anteriores, ya sabemos que con CI. nulas, el $S_2$
...
%        y_1(t) \neq y_2(t)
%      \end{gather*}
%      Si multiplicamos la salida de ambas funciones por $\mu_t$ para que el sistema sea causal, nos quedaría lo
%      siguiente:
%      \begin{gather*}
...
%      podemos re acomodar la integral para que quede igual que $y_1(t)$:
%      \begin{equation*}
...
%      por lo tanto, cuando $v < t_0$, la señal de entrada va a ser evaluada en $t < 0$, por lo que su salida va a ser
%      nula debido a la causalidad. Cuando $v = t_0$, la señal de entrada va a ser evaluada en 0, pero
%      independientemente de lo que valga, el aporte a la integración va a ser nulo. Esto permite 
    ...
    \item Comparar las respuestas del sistema linealizado $y_t = S_3\{x_t\}$ con su formula en convolución,
      ...
      Los desarrollos de este ejercicio resultaron ser bastante repetitivos y en la mayoría de los casos las
      propiedades utilizadas eran las mismas. Las únicas diferencias en los desarrollos fueron algunas integrales que
      se convirtieron en integrales por partes. A continuación esta el cuadro de comparación:
      ...
      El correspondiente diagrama de simulación es:
      ...
      \begin{figure}[h]
        ...
        \begin{tikzpicture}[auto, node distance=2cm, >=Stealth, thick]
          ...
          \node [block, below=1cm of elbow1] (gain1) {$\frac{1}{R}$};
          \node [sum, below=1cm of gain1] (sum) {$+$};
          \node [block, left=2cm of sum] (integrator) {$\frac{1}{s}$};
          \node [coordinate, left=2cm of integrator] (elbow2) {};
          \node [block, below=1cm of elbow2] (gain2) {$-\frac{1}{RC}$};
          \node [coordinate, below=1cm of gain2] (elbow3) {};
          \node [coordinate, right=4cm of sum] (output) {}; % Nodo de salida
          ...
          % Conexiones entre nodos
          \draw [-] (input) -- node {$X_s$} (elbow1);
          \draw [-] (elbow1) -- (gain1);
          \draw [->] (gain1) -- (sum);
          \draw [->] (sum) -- node {$Y_s$} (output);
          \draw [->] (integrator) -- (sum);
          \draw [->] (elbow2) -- (integrator);
          \draw [-] (gain2) -- (elbow2);
          \draw [->] (elbow3) -- (gain2);
          ...
          \draw [-] (elbow5) -- (elbow4);
        \end{tikzpicture}
        $$Y_s = \frac{X_s}{R} + \frac{1}{s}\left(-\frac{Y_s}{RC}\right)$$
      \end{figure}
      ...
      Ademas, ahora podemos también determinar los valores para $y(0)$ e $y(\infty)$ utilizando la transformada de
      ...
      \end{align*}
      Los valores obtenidos para el sistema tienen mucho sentido. Si le damos un análisis físico, tiene sentido que
      cuando el circuito recibe el impulso, la corriente tiende a infinito, debido a que el capacitor "funciona" como
      un cortocircuito. Obviamente, en una situación real, esto no se iría al infinito, si no que solamente tendería a
      $\frac{V}{R}$, de tal forma que el capacitor actúa como un cable.
      Así mismo, cuando la función tiende al infinito, la corriente tiende a cero, debido a que ya el capacitor debería
      estar completamente cargado, comportándose como un circuito abierto, y evitando la totalidad del paso de la
  ...
  \begin{enumerate}[label=\alph*)]
    ...
    \item Obtener una representación del circuito con Ecuaciones Diferenciales de segundo orden y coeficientes constantes,
      ...
      El circuito RLC esta descrito por la ecuación característica:
      \begin{equation*}
        v(t) = Ri_{(t)} + L \frac{d}{dt} i_{(t)} + \frac{1}{C} \int_{0}^{t} i_{(\tau)} d\tau
      \end{equation*}
      Para tener una nomenclatura mas genérica, sustituimos la entrada $v(t)$ con $x(t)$ y la salida $i(t)$ con $y(t)$:
      \begin{equation*}
      ...
      \end{align*}
      Lo cual es la ecuación diferencial de segundo orden a coeficientes constantes que describe el comportamiento
      del circuito RCL.

      Para el diagrama de simulación solo tuvimos que integrar la ecuación diferencial hasta que no hubo mas derivadores:
      ...
      \begin{figure}[h]
        ...
        \begin{tikzpicture}[auto, node distance=2cm, >=Stealth, thick]
          ...
          \node [block, on grid, below=1cm of elbowInput1] (gainInput1) {$\frac{1}{L}$};
          ...
          \node [coordinate, on grid, below=1.5cm of gainOutput1] (elbowOutput2) {};
          \node [connector, on grid, below=1.5cm of gainOutput2] (elbowOutput3) {};
          \node [coordinate, on grid, right=5cm of elbowOutput3] (elbowOutput4) {};
          \node [coordinate, on grid, above=3.5cm of elbowOutput4] (elbowOutput5) {};
          \node [coordinate, on grid, right=2cm of elbowOutput5] (output) {};

          % Conexiones entre nodos
          \draw [>-] (input) -- node {$x_t$} (elbowInput1);
          ...
          \draw [->] (elbowOutput5) -- node {$y_t$} (output);
        ...
        \end{tikzpicture}
        \begin{equation*}
          y_t = D^{-2}\left\{-\frac{1}{LC} y_t\right\} + D^{-1}\left\{\frac{1}{L} x_t - \frac{R}{L} y_t\right\}
        \end{equation*}
      \end{figure}
    ...
    \item Sea $y_t = S\{x_t\}$ el sistema obtenido de las soluciones al sistema de Ecuaciones Diferenciales del inciso
      ...
      \end{equation*}
      Por lo tanto, la solución general explicita de la ecuación diferencial transformada es:
      \begin{equation*}
        Y_s = \frac{\frac{1}{L}s}{s^2 + \frac{R}{L}s + \frac{1}{LC}} X_s + \frac{y_0 s +
          \frac{R}{L} y_0 - \frac{1}{L} x_0 + y_0'}{s^2 + \frac{R}{L}s + \frac{1}{LC}}
      \end{equation*}
      Esto se puede verificar, tomando la ecuación diferencial de segundo orden a coeficientes constantes, y aplicándole
      su transformada de Laplace, utilizando la propiedad de derivación en el dominio del tiempo, y luego despejando para
      $Y_s$:
      \begin{gather*}
    ...
    \item Restringir el sistema anterior a CI nulas y obtener la formula de la respuesta transformada $Y_{(s)} = \mathcal{L}\{y\}$.
      ...
      La nueva transformada con CI. nulas es la siguiente:

      \begin{equation*}
        Y_s = \frac{\frac{s}{L}}{s^2 + \frac{R}{L} s + \frac{1}{LC}} X_s
      \end{equation*}

      La función de transferencia $H_s$ se obtiene despejando la función anterior para $\frac{Y_s}{X_s}$:
  ...
  \newpage
    ...
    \item Graficar en diagrama de bloques del sistema en el dominios de la variable compleja.  Determinar el coeficiente
      ...
      algún significado subrayar que el diagrama esta en el dominio del tiempo $(- \infty , \infty)$, o en el dominio complejo?
      ...
      \begin{figure}[h]
        ...
        \begin{tikzpicture}[auto, node distance=2cm, >=Stealth, thick]
          ...
          \node [block, on grid, below=1cm of elbowInput1] (gainInput1) {$\frac{1}{L}$};
          \node [sum, on grid, below=2cm of gainInput1] (sum1) {$+$};
          \node [block, on grid, left=3cm of sum1] (integrator1) {$\frac{1}{s}$};
          \node [block, on grid, right=3cm of sum1] (integrator2) {$\frac{1}{s}$};
          \node [connector, on grid, left=3cm of integrator1] (elbowOutput1) {};
          \node [block, on grid, below=2cm of elbowOutput1] (gainOutput1) {$-\frac{1}{LC}$};
          \node [block, on grid, below=2cm of sum1] (gainOutput2) {$-\frac{R}{L}$};
          \node [coordinate, on grid, below=1.5cm of gainOutput1] (elbowOutput2) {};
          ...
          % Conexiones entre nodos
          \draw [>-] (input) -- node {$X_s$} (elbowInput1);
          \draw [->] (elbowInput1) -- (gainInput1);
          \draw [->] (gainInput1) -- (sum1);
          \draw [->] (elbowOutput1) -- (integrator1);
          \draw [->] (integrator1) -- (sum1);
          \draw [->] (sum1) -- (integrator2);
          \draw [-] (integrator2) -- (elbowOutput5);
          \draw [->] (elbowOutput5) -- node {$Y_s$} (output);
          \draw [-] (elbowOutput1) -- (gainOutput1);
          \draw [->] (elbowOutput2) -- (gainOutput1);
          \draw [-] (elbowOutput2) -- (elbowOutput3);
          \draw [-] (elbowOutput3) -- (elbowOutput4);
