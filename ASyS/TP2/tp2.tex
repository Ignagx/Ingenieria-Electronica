...
\begin{enumerate}[label=\alph*),left=0pt]
  ...
  \item Determinar las ecuaciones de cinemáticas para la posición $y_1(t)$ y $y_2(t)$ en función del tiempo para cada
    ...
    las masas no intervienen en la descripcion de las funciones.
...
\begin{enumerate}[label=\alph*),left=0pt]
  ...
  \item Determinar la ecuación de cinemática para la posición $x(t)$ en función del tiempo. \textbf{Reflexionar:} ¿la
    ...
    Partiendo de la ecuacion diferencial de un oscilador armonico simple $m \frac{dx^2}{d^2t} + kx = 0$, determinamos
  ...
  \begin{enumerate}[label=\alph*)]
    ...
    Tiene energia infinita si consideramos el intervalo $(-\infty,\infty)$, pero la energia es finita en un solo
  ...
  \item Obtener la transformada de Fourier $F_1(\omega) = \mathcal{F}\{x_1\}$ y similarmente $F_2(\omega) =
    ...
    \begin {itemize}[left=0pt]
      ...
      \item Calcular la energía $E_1, E_2$ haciendo uso de la relación de Parseval.
        ...
        Siendo la energía de cada señal a través de la relacion de persaval:
        ...
        En este caso, el metodo sobre la variable $t$ es mucho más simple, directo y visual. En contra parte el calculo
...
%      independientemente de lo que valga, el aporte a la integracion va a ser nulo. Esto permite 
    ...
    \item Comparar las respuestas del sistema linealizado $y_t = S_3\{x_t\}$ con su formula en convolución,
        ...
        \begin{tabular}{|c|c|c|c|}
          ...
          \hline
            $\mu_{t-1}$ & $\frac{1}{R}\left(\mu_{(t-1)} - 1 + e^{\frac{1-t}{RC}}\right) \mu_t$
                        & $\frac{1}{R} \left(\mu_{(t-1)} + 1 - e^{\frac{1-t}{RC}}\right)$%$\frac{1}{R}\left(\mu_{(t-1)} - 1 + e^{\frac{1-t}{RC}}\right) \mu_t$
                        & Diferente\\
          \hline
            $\mu_t - \mu_{t-1}$ & $\frac{1}{R} \left[\mu_t \left(1 + e^{\frac{-t}{RC}} - e^{\frac{1-t}{RC}}\right) -
            \mu_{(t-1)} \right]$
                                & $ \frac{1}{R}\left(\mu_t - \mu_{(t-1)} - 2 + e^{\frac{-t}{RC}} + e^{\frac{1-t}{RC}}\right)$%$\frac{1}{R} \left[\mu_t \left(1 + e^{\frac{-t}{RC}} - e^{\frac{1-t}{RC}}\right) -
           %\mu_{(t-1)} \right]$
                                & Diferente\\
          \hline
      ...
      Como se pudo ver, ningun sistema admite la descripcion por convolucion, excepto el sistema donde $x_t = \delta_t$.
      Esto es porque el sistema no es invariante en el tiempo. Anteriormente mostramos que es posible que el sistema se
      comporte como un LIT, causalizando su entrada. Por lo tanto, debido a la no invarianza en el tiempo del sustema,
      cualquier representacion por convolucion va a ser erronea, debido a que no asegura que la respuesta al sea la
      misma para cualquier desplazamiento temporal del sistema, lo cual es la base de la convolucion. Ademas, el sistema
      donde $x_t = t$ no es comparable, debido a que su convolucion tiende a infinito, debido a que $t$, no es
      ...
      impulso y al escalon unidad obtenidas a partir de $S_3\{x_t\}$:
